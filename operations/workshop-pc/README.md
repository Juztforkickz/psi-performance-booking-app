...
It creates the upload root and a **PSI Workshop Uploads** desktop shortcut for the signed-in Windows user. It creates one Windows Startup shortcut for the bottom-right PSI notification-area app. That single tray app starts automatic watching silently, shows its status and opens the centred five-option menu on demand. Opening or closing the menu does not pause or stop automatic watching. The installer removes the two older watcher/menu Startup shortcuts so duplicate blue and black console windows cannot return. The saved configuration contains only the public project address, publishable key, staff email, upload path and Downloads inbox path. Customer records and privileged keys are not stored in it.

The tray app checks the approved public PSI repository for a versioned workshop-app update every six hours. It only shows a notification; it never silently installs executable code. Choose **Install workshop app update** and approve the Windows prompt to pause the watcher, download the allowlisted files, verify every SHA-256 hash, back up the current installation, run Python and PowerShell startup checks, and restart syncing. `config.json`, `session.dpapi`, upload progress and `C:\PSI Uploads` are outside the update file list and remain untouched. A failed startup check restores the previous files from `.update-backup` and records `last-update-error.log`.
