-- A cancelled request has no amount currently due. Verified payments will be
-- represented by the future payment ledger, not by this expected-deposit field.
...
begin
  ...
  elsif new.state in ('pending_staff_review', 'date_proposed', 'cancelled') then
    new.deposit_amount_cents := null;
  end if;
  new.currency := 'AUD';
  return new;
...
-- Existing controlled-QA cancellations predate this correction. No payment
-- provider is enabled, so these values represent an amount that was never due.
select set_config('request.jwt.claims', '{"role":"service_role"}', true);

update public.booking_requests
set deposit_amount_cents = null
