...
export function PerformanceUpdates() {
 ...
 return <View style={s.pricing}><Text style={s.section}>Your vehicle record</Text>{state.rows.map(row=><Pressable accessibilityRole="button" key={row.job_id} onPress={()=>router.push({pathname:'/performance-plus',params:{vehicleId:row.vehicle_id}})}><Text style={s.copy}>Your PSI vehicle record has been updated.</Text><Text style={s.link}>{row.record_count} {row.record_count === 1 ? 'record' : 'records'} available · {new Date(row.updated_at).toLocaleDateString('en-AU')}</Text></Pressable>)}</View>;
}
