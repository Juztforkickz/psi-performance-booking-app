...
export default function PerformancePlusScreen() {
  ...
  const refresh = async () => {
    if (busy) return;
    setBusy(true);
    try { if (!permanentPlus && purchasesAvailable && auth.user) await verifyWithServer(); setMessage(''); }
  ...
  return <SafeAreaView edges={['top', 'left', 'right']} style={s.screen}><ScrollView contentContainerStyle={[s.content, { paddingHorizontal: horizontalPadding }]}>
    ...
    {!activePlus && entitlementReady ? <View style={s.pricing}><Text style={s.pricingEyebrow}>UNLOCK YOUR COMPLETE VEHICLE STORY</Text><Text style={s.section}>Choose Performance+</Text><Text style={s.copy}>One subscription covers every vehicle in your PSI account.</Text>
      <View style={[s.priceGrid, singleColumn && s.priceGridStacked]}>
        <Pressable accessibilityLabel={`${aud(PERFORMANCE_PRICING.monthly)} monthly`} accessibilityRole="radio" accessibilityState={{ checked: selectedPeriod === 'monthly', disabled: busy }} disabled={busy} onPress={() => { setSelectedPeriod('monthly'); setMessage(''); }} style={({ pressed }) => [s.priceOption, selectedPeriod === 'monthly' && s.priceOptionSelected, pressed && s.pressed]}><View style={s.priceChoiceHeading}><View style={s.priceChoiceLabels}><Text style={s.priceLabel}>MONTHLY OPTION</Text><Text style={s.price}>{aud(PERFORMANCE_PRICING.monthly)} monthly</Text></View><Ionicons name={selectedPeriod === 'monthly' ? 'radio-button-on' : 'radio-button-off'} color={colors.accent} size={20} /></View><Text style={s.priceMeta}>Billed every month</Text></Pressable>
        <Pressable accessibilityLabel={`${aud(PERFORMANCE_PRICING.annual)} annual, best value`} accessibilityRole="radio" accessibilityState={{ checked: selectedPeriod === 'annual', disabled: busy }} disabled={busy} onPress={() => { setSelectedPeriod('annual'); setMessage(''); }} style={({ pressed }) => [s.priceOption, s.bestValue, selectedPeriod === 'annual' && s.priceOptionSelected, pressed && s.pressed]}><View style={s.priceChoiceHeading}><View style={s.priceChoiceLabels}><Text style={s.bestValueLabel}>BEST VALUE</Text><Text style={s.priceLabel}>ANNUAL OPTION</Text><Text style={s.price}>{aud(PERFORMANCE_PRICING.annual)} annual</Text></View><Ionicons name={selectedPeriod === 'annual' ? 'radio-button-on' : 'radio-button-off'} color={colors.accent} size={20} /></View><Text style={s.priceMeta}>Billed once a year · save {aud(PERFORMANCE_PRICING.monthly * 12 - PERFORMANCE_PRICING.annual)}</Text></Pressable>
      </View>
      ...
      <PrimaryButton disabled={!purchasesAvailable} loading={busy} label={`Continue with ${aud(PERFORMANCE_PRICING[selectedPeriod])} ${selectedPeriod}`} onPress={() => void subscribe(selectedPeriod)} />
...
export const s = StyleSheet.create({
  ...
  priceChoiceLabels: { alignItems: 'flex-start', gap: 5 },
  ...
  selectionHelp: { color: colors.silver, fontSize: 13, lineHeight: 20 },
