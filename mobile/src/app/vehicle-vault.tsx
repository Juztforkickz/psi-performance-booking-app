...
export default function VehicleVault() {
  ...
  return <SafeAreaView style={s.screen}><ScrollView contentContainerStyle={s.content}>
    ...
    {locked ? <View style={s.pricing}><Text style={s.section}>Your vehicle story is here</Text><Text style={s.copy}>Unlock Performance+ to view the premium records. Your bookings, kilometres and maintenance reminders remain free.</Text><PrimaryButton label="Explore Performance+" onPress={() => router.replace({ pathname: '/performance-plus', params: { vehicleId } })} /></View> : null}
  ...
  </ScrollView><Modal visible={!locked && image?.key === key && (!CUSTOMER_AUTH.enabled || auth.status === 'signed_in')} onRequestClose={() => setImage(null)}><SafeAreaView style={s.screen}><PrimaryButton label="Close photo" onPress={() => setImage(null)} />{image?.key === key ? <Image source={{ uri: image.url }} resizeMode="contain" style={{ flex: 1 }} /> : null}</SafeAreaView></Modal></SafeAreaView>;
}
