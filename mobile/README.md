...
The first PSI staff-portal foundation is available at `/staff` only in an Auth-enabled private QA build. It verifies the signed-in identity against the existing staff allowlist, checks Supabase Auth's authenticator assurance level, and loads no workshop-wide records until the session is AAL2. An active MFA-verified staff session receives a read-only booking queue plus customer and vehicle lookup through the existing RLS policies. The public build fails closed, the route is not linked from customer navigation, and staff publishing, edits, files, Calendar actions and staff management remain disabled.
