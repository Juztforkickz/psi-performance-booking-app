...
The account, garage, bookings and alerts routes remain a UI preview and do not yet load or write real customer records. Supabase is now the selected app identity/data foundation: a Sydney project has RLS-protected customer/workshop tables, a staff allowlist and private file buckets. The Expo client includes a fail-closed Supabase adapter for six-digit passwordless email codes, PKCE and native OS-protected session storage; its activation flag remains false until PSI's custom SMTP sender and end-to-end access tests pass. The public web preview keeps future sessions in memory only. No password field exists, and the preview still performs no account creation, notification or file upload. See `../docs/SUPABASE-APP-FOUNDATION.md` for the exact roles, ownership rules, email and Google Calendar gates; the existing root web account endpoints remain disabled as described in `../docs/CUSTOMER-ACCOUNTS.md`.
