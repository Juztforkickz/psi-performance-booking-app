...
and authenticated worker are present, but provider delivery, customer
...
the later trusted confirmed transition creates email plus Calendar jobs. The
worker cannot deliver them until its private provider credentials are added and
tested. Public GitHub Pages
