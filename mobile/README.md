...
The dashboard and public customer tabs currently use clearly labelled synthetic, in-memory examples for the account, vehicles, bookings, alerts, dyno results and build plan. Validated preview account fields and the selected vehicle photo, active vehicle selection, booking prefill and Plan & Build handoff travel only through the root in-memory preview context. My Garage shows the customer odometer beside the separate PSI service/check-in history; personal service and check-in fields are omitted. In an authenticated QA build, the Garage reads protected PSI service/check-in dates, PSI odometer figures and the latest customer odometer from the owned account, adds a new customer odometer as an append-only RLS-scoped record, and stores a customer-selected vehicle photo under that customer's UUID in the private `vehicle-photos` bucket. Photo metadata is ownership-checked, image-only, limited to 8 MB and displayed with a short-lived signed URL; authorised AAL2 PSI staff may view it for workshop use. Public-preview values put no personal information in route URLs and perform no upload or persistence; the context clears when the app reloads or closes. This preview context is separate from the explicitly saved 30-day booking-form draft described below. PSI-verified dyno figures remain read-only to the customer.

Vehicle Reports has its own full-screen `/vehicle-reports` workspace, separate from My Garage. In the public preview it filters synthetic PSI-style history and locally added preview records by the selected vehicle; preview additions clear with the demo. In an authenticated build, the four Add workflows save private customer dyno results, previous repairs, future-work notes and invoice records under row-level ownership controls. PSI Free retains the vehicle details, service dates, work summaries, recommendations, bookings and service reminders. Performance+ adds access to the attached workshop photos, invoice copies, dyno graph images/PDFs, supporting documents, downloads and detailed organised archive. Xero continues to email the original invoice to every customer regardless of the app plan. Saved customer records and linked documents are append-only and clearly labelled as customer-provided, while PSI-verified dyno results and PSI workshop records remain separately labelled and read-only. Invoice amounts remain AUD. Eligible private image attachments open in-app through a short-lived authorized URL with an explicit return action; PDFs open through the device browser. The URL remains only in screen memory.

The Stage 2 account foundation relies on managed authentication, verified ownership and database row-level controls already established for private QA. Customers receive customer-scoped reads and may append clearly labelled private report entries for vehicles they own; they cannot revise those entries after saving or publish PSI-verified records. Public registration and the public production booking API remain disabled; only the separately gated authenticated QA booking path is active. A separate AAL2-only workshop control allows an active PSI staff identity to publish PSI repair history, recommended work, verified dyno results and AUD invoice metadata. Optional invoice and dyno files use unique customer-prefixed paths in the private `vehicle-documents` bucket and require either an active Performance+ entitlement or active PSI staff access at both metadata and object-storage layers. Vehicle profile photos remain available with PSI Free in the separate private `vehicle-photos` bucket. The client creates no public URL and attempts to remove an unused object when its matching record cannot be created, surfacing a warning if cleanup fails.
