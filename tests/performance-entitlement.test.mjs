...
const products = ['plus.monthly', 'plus.annual'];
const make = () => ({ original_app_user_id: 'customer-a', entitlements: { performance_plus: { product_identifier: products[0], expires_date: '2026-10-09T00:00:00Z' } }, subscriptions: { [products[0]]: { store: 'app_store', is_sandbox: false, ownership_type: 'PURCHASED', refunded_at: null, unsubscribe_detected_at: null, grace_period_expires_date: null } } });
...
test('cancellation keeps paid access until expiry', () => {
  const value = make(); value.subscriptions[products[0]].unsubscribe_detected_at = '2026-09-08T00:00:00Z';
...
test('verified billing grace lasts only through provider grace expiry', () => {
  ...
  value.subscriptions[products[0]].grace_period_expires_date = '2026-09-10T00:00:00Z';
  assert.equal(check(value).status, 'grace_period');
  value.subscriptions[products[0]].grace_period_expires_date = '2026-09-08T12:00:00Z';
  assert.equal(check(value).status, 'expired');
});
test('refund revokes even when a stale future expiry exists', () => {
  const value = make(); value.subscriptions[products[0]].refunded_at = '2026-09-08T00:00:00Z';
...
test('sandbox cannot grant production entitlement', () => {
  const value = make(); value.subscriptions[products[0]].is_sandbox = true;
...
test('unknown products, other stores and shared receipts do not unlock', () => {
  ...
  for (const store of ['stripe', 'promotional', 'play_store']) {
    const value = make(); value.subscriptions[products[0]].store = store;
    assert.equal(check(value).status, 'expired');
  }
  const shared = make(); shared.subscriptions[products[0]].ownership_type = 'FAMILY_SHARED';
...
test('missing entitlement, invalid expiry and unknown environment fail closed', () => {
  ...
  const environment = make(); delete environment.subscriptions[products[0]].is_sandbox;
