...
The pre-subscription tag `psi-beta-before-performance-plus-2026-09-09` remains intact. The immediate pre-expansion source checkpoint is `b8d6b2a`. This change adds JavaScript and static artwork only; it requires no native module or runtime change. The beta OTA update uses the exact build-9 runtime `1.0.0-beta-performance-plus-1` and beta channel. Build 7's different runtime remains unaffected. Public GitHub Pages continues through the existing workflow; no Netlify deployment is involved.

Library feature checkpoint: `50d57552b57981d5b4c7fa009eaccc97f4898edc`. The [GitHub Pages deployment](https://github.com/Juztforkickz/psi-performance-booking-app/actions/runs/34294889171) passed. A live browser check opened the compact selector, confirmed the 54-illustration count and make filters, searched `ram 1500`, and rendered the RAM 1500 thumbnail and `DT · 2022` label. [Open the garage preview](https://juztforkickz.github.io/psi-performance-booking-app/garage).

The live web console also reproduces an existing recoverable first-load React hydration repaint at the first Ionicons glyph. The Garage screen and picker source are unchanged between the prior and current artwork checkpoints; the client immediately recovers, and selection/search remain functional. This is limited to the static web preview's icon-font startup path and is not a native beta failure. A root-level icon-font hydration refinement should be handled separately because changing the shared first render affects every route.

The native iOS export also passed and the [54-car beta update](https://expo.dev/accounts/psi-performance/projects/matt-psi/updates/9a8362c8-5358-4335-bc6e-4517d179d295) was published successfully: group `9a8362c8-5358-4335-bc6e-4517d179d295`, iOS update `01a0838f-085a-7ace-842e-c58f6e0a6a22`, beta channel/branch, runtime `1.0.0-beta-performance-plus-1`. EAS uploaded the changed bundle and 32 assets while reusing 150 existing assets. Only compatible iOS builds can receive it; build 7 keeps its previous `1.0.0-beta-demo-1` update. No new binary was submitted and no subscriptions were activated for this refinement. Installed-device receipt and interaction testing remain to be confirmed.
