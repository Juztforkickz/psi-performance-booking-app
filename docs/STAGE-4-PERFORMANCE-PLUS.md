...
PSI can prepare a workshop-only customer, vehicle, job and files before the customer downloads the app. The customer then creates and completes their own verified PSI account. That customer action starts the one-time 14-day Performance+ trial and an exact verified-email match automatically connects eligible prepared workshop records and vehicles to the account. Creating the workshop contact, inviting the email, and publishing galleries do not start the countdown. The trial unlocks the full Performance+ experience across every vehicle in the account, takes no payment and does not renew. Completing or editing the profile again cannot restart or extend it. After expiry the account returns to PSI Free while the private archive remains stored and locked; a later paid subscription reopens the same archive.
