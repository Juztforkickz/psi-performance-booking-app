...
`performance_subscriptions` is server controlled. Clients may read their own status; they cannot insert, update or grant entitlements. Paid, grace-period and complimentary records require an expiry. Turning off renewal retains the paid period. Expiry returns records to locked state without deletion. Sandbox receipts cannot activate the main project. The verified PSI owner/customer account is represented separately by a production-only permanent complimentary entitlement at A$0 with no renewal or expiry.

The workshop PC can create an account-optional customer, vehicle and job folder before the customer downloads the app. Files placed in that folder are prepared locally and remain private with status `waiting_for_customer_account`. The customer then creates and completes their own verified PSI account and vehicle. That customer action starts the one-time 14-day Performance+ trial. A matching registration plus either verified email, or exact full name and normalized mobile, automatically transfers the eligible workshop job and vehicle to the customer. The PC watcher upgrades the same verified folder manifest and uploads the waiting files on its next scan. Anything without that deterministic match remains staff-only for owner review. Creating the workshop folder does not start the countdown. The trial takes no payment and does not renew; completing or editing the profile again cannot restart or extend it. After expiry the account returns to PSI Free while the private archive remains stored and locked; a later paid subscription reopens the same archive.
