...
When PSI publishes the first workshop photo gallery for an active customer, the database starts that customer’s one-time 14-day Performance+ trial. It unlocks the full Performance+ experience across every vehicle in the account, takes no payment and does not renew. Publishing more galleries cannot restart or extend the trial. After expiry the account returns to PSI Free while the private archive remains stored and locked; a later paid subscription reopens the same archive. The MFA-verified owner has a portal fallback to start the same one-time trial for an existing customer whose first gallery predated this automation.
