...
No live payment or provider credential is configured in source. The controlled mobile QA path can store and review a private request and now records durable, deduplicated email/Calendar jobs. Its authenticated Edge Function refuses delivery while the private Resend and Google credentials are absent. It cannot take money or send a deposit link, and Google Calendar work is queued only after the future trusted payment-confirmed transition. Payment checkout is created only from the staff approval workflow, and a signed payment-provider webhook is the only production path that may mark a deposit paid.

Booking data is stored in the D1 binding named `DB`. The schema is in `db/schema.ts`; generated migrations are under `drizzle/`.

The existing web customer-account groundwork remains deliberately fail-closed. `GET /api/v1/account/status` reports that authentication and registration are disabled, while profile reads and writes return `503 CUSTOMER_ACCOUNTS_DISABLED`. The mobile app now has a separate Supabase foundation for passwordless email codes, RLS-protected Sydney data and private files. Its controlled QA profile connects approved authenticated accounts to an idempotent private booking queue, customer-owned private vehicle photos, MFA-protected PSI date review and a fail-closed integration outbox, while the public preview remains submission-disabled. Payment confirmation is still unimplemented; transactional booking email and Google Calendar remain server-gated and cannot deliver until their private provider credentials are configured and accepted. See [`docs/CUSTOMER-ACCOUNTS.md`](docs/CUSTOMER-ACCOUNTS.md) and [`docs/SUPABASE-APP-FOUNDATION.md`](docs/SUPABASE-APP-FOUNDATION.md).
