...
No live payment or provider credential is stored in source. The controlled mobile QA path can store and review a private request and records durable, deduplicated email/Calendar jobs. Resend email delivery and the least-privilege Google Calendar connection are configured only as encrypted Edge Function secrets. The booking-specific worker now attempts request/status emails immediately and retains the AAL2-only retry queue. It cannot take money or send a deposit link, and Google Calendar work is queued only after the future trusted payment-confirmed transition. A signed payment-provider webhook remains the only planned production path that may mark a deposit paid.

Booking data is stored in the D1 binding named `DB`. The schema is in `db/schema.ts`; generated migrations are under `drizzle/`.

The existing web customer-account groundwork remains deliberately fail-closed. `GET /api/v1/account/status` reports that authentication and registration are disabled, while profile reads and writes return `503 CUSTOMER_ACCOUNTS_DISABLED`. The mobile app now has a separate Supabase foundation for passwordless email codes, RLS-protected Sydney data and private files. Its controlled QA profile connects approved authenticated accounts to an idempotent private booking queue, customer-owned private vehicle photos, MFA-protected PSI date review and a fail-closed integration outbox, while the public preview remains submission-disabled. Transactional booking email is active only in private QA. Google Calendar remains server-gated and receives no event until the future payment-confirmed transition; payment confirmation itself is still unimplemented. See [`docs/CUSTOMER-ACCOUNTS.md`](docs/CUSTOMER-ACCOUNTS.md) and [`docs/SUPABASE-APP-FOUNDATION.md`](docs/SUPABASE-APP-FOUNDATION.md).
