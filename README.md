...
Customer-account groundwork is deliberately fail-closed. `GET /api/v1/account/status` reports that authentication and registration are disabled, while profile reads and writes return `503 CUSTOMER_ACCOUNTS_DISABLED`. The approved direction is invite-only testing with passwordless email links; PSI will store profile and vehicle data in D1 but no password, sign-in token or session secret. See [`docs/CUSTOMER-ACCOUNTS.md`](docs/CUSTOMER-ACCOUNTS.md) for the provider-selection and activation gates.
