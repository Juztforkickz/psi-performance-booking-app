...
The existing web customer-account groundwork remains deliberately fail-closed. `GET /api/v1/account/status` reports that authentication and registration are disabled, while profile reads and writes return `503 CUSTOMER_ACCOUNTS_DISABLED`. The mobile app now has a separate Supabase foundation for passwordless email codes, RLS-protected Sydney data and private files. Its controlled QA profile connects approved authenticated accounts to an idempotent private booking queue and MFA-protected PSI date review, while the public preview remains submission-disabled. Payment confirmation, transactional booking email and Google Calendar are still server-gated. See [`docs/CUSTOMER-ACCOUNTS.md`](docs/CUSTOMER-ACCOUNTS.md) and [`docs/SUPABASE-APP-FOUNDATION.md`](docs/SUPABASE-APP-FOUNDATION.md).
